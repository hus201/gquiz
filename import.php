...
$url = new moodle_url('/mod/gquiz/import.php', array('id'=>$id));
if ($choosefile !== false) {
...
if ($choosefile) {
    ...
    if (!$xmldata = gquiz_load_xml_data($xmlcontent)) {
        print_error('cannotloadxml', 'gquiz', 'edit.php?id='.$id);
    }

    $importerror = gquiz_import_loaded_data($xmldata, $gquiz->id);
    if ($importerror->stat == true) {
        $url = 'edit.php?id='.$id.'&do_show=templates';
        redirect($url, get_string('import_successfully', 'gquiz'), 3);
        exit;
    }
}
...
///////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////
echo $OUTPUT->heading(get_string('import_questions', 'gquiz'), 3);
...
function gquiz_load_xml_data($xmlcontent) {
    global $CFG;
    require_once($CFG->dirroot.'/lib/xmlize.php');

    if (!$xmlcontent = gquiz_check_xml_utf8($xmlcontent)) {
        return false;
    }

    $data = xmlize($xmlcontent, 1, 'UTF-8');

    if (intval($data['gquiz']['@']['VERSION']) != 200701) {
        return false;
    }
    $data = $data['gquiz']['#']['ITEMS'][0]['#']['ITEM'];
    return $data;
}

function gquiz_import_loaded_data(&$data, $gquizid) {
    global $CFG, $DB;

    gquiz_load_gquiz_items();
    ...
    if (!is_array($data)) {
        $error->msg[] = get_string('data_is_not_an_array', 'gquiz');
        $error->stat = false;
        return $error;
    }

    if ($deleteolditems) {
        gquiz_delete_all_items($gquizid);
        $position = 0;
    } else {
        //items will be add to the end of the existing items
        $position = $DB->count_records('gquiz_item', array('gquiz'=>$gquizid));
    }
    ...
    foreach ($data as $item) {
        ...
        $newid = $DB->insert_record('gquiz_item', $newitem);
...
function gquiz_check_xml_utf8($text) {
    //find the encoding
