...
/**
 * Steps definitions related to mod_gquiz.
 *
 * @package   mod_gquiz
 * @category  test
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_mod_gquiz extends behat_base {

    /**
     * Adds a question to the existing gquiz with filling the form.
     *
     * The form for creating a question should be on one page.
     *
     * @When /^I add a "(?P<question_type_string>(?:[^"]|\\")*)" question to the gquiz with:$/
     * @param string $questiontype
     * @param TableNode $questiondata with data for filling the add question form
     */
    public function i_add_question_to_the_gquiz_with($questiontype, TableNode $questiondata) {

        $questiontype = $this->escape($questiontype);
        $additem = $this->escape(get_string('add_item', 'gquiz'));
        ...
        $saveitem = $this->escape(get_string('save_item', 'gquiz'));
     ...
     * The form for creating a question should be on one page.
     *
     * @When /^I add a page break to the gquiz$/
     */
    public function i_add_a_page_break_to_the_gquiz() {

        $questiontype = $this->escape(get_string('add_pagebreak', 'gquiz'));
        $additem = $this->escape(get_string('add_item', 'gquiz'));
    ...
    /**
     * Quick way to generate answers to a one-page gquiz.
     *
     * @When /^I log in as "(?P<user_name_string>(?:[^"]|\\")*)" and complete gquiz "(?P<gquiz_name_string>(?:[^"]|\\")*)" in course "(?P<course_name_string>(?:[^"]|\\")*)" with:$/
     * @param string $questiontype
     * @param TableNode $questiondata with data for filling the add question form
     */
    public function i_log_in_as_and_complete_gquiz_in_course($username, $gquizname, $coursename, TableNode $answers) {
        $username = $this->escape($username);
        $coursename = $this->escape($coursename);
        $gquizname = $this->escape($gquizname);
        $completeform = $this->escape(get_string('complete_the_form', 'gquiz'));
    ...
    /**
     * Exports gquiz and makes sure the export file is the same as in the fixture
     *
     * @Then /^following "(?P<link_string>(?:[^"]|\\")*)" should export gquiz identical to "(?P<filename_string>(?:[^"]|\\")*)"$/
     * @param string $link
     * @param string $filename
     */
    public function following_should_export_gquiz_identical_to($link, $filename) {
        global $CFG;
     ...
     *
     * @Then /^I show chart data for the "(?P<gquiz_name_string>(?:[^"]|\\")*)" gquiz$/
     * @param string $gquizname name of the gquiz for which chart data needs to be shown.
     */
    public function i_show_chart_data_for_the_gquiz($gquizname) {

        $gquizxpath = "//th[contains(normalize-space(string(.)), \"" . $gquizname . "\")]/ancestor::table/" .
        ...
        $charttabledataxpath = $gquizxpath .
    ...
    /**
     * Ensures two gquiz export files are identical
     *
    ...
    protected function compare_exports($expected, $actual) {
        ...
        $dataexpected = $dataexpected['gquiz']['#']['ITEMS'][0]['#']['ITEM'];
        $dataactual = xmlize($actual, 1, 'UTF-8');
        $dataactual = $dataactual['gquiz']['#']['ITEMS'][0]['#']['ITEM'];
