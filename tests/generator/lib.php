...
/**
 * mod_gquiz data generator class.
 *
 * @package    mod_gquiz
 * @category   test
 * @copyright  2013 Ankit Agarwal
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_gquiz_generator extends testing_module_generator {
    ...
    public function create_instance($record = null, array $options = null) {
        ...
        if (!isset($record->anonymous)) {
            $record->anonymous = gquiz_ANONYMOUS_YES;
        }
        ...
        // Hack to bypass draft processing of gquiz_add_instance.
     ...
     *
     * @param object $gquiz gquiz record
     * @param array $record (optional) to override default values
     * @return int
     */
    public function create_item_info($gquiz, $record = array()) {
        global $DB, $CFG;

        require_once($CFG->dirroot.'/mod/gquiz/lib.php');

        $itemobj = gquiz_get_item_class('info');
        $position = $DB->count_records('gquiz_item', array('gquiz' => $gquiz->id)) + 1;

        $record = (array)$record + array(
            'id' => 0,
            'gquiz' => $gquiz->id,
            'template' => 0,
            'name' => 'gquiz question item ' . $position,
            'label' => 'gquiz label ' . $position,
     ...
     * @return int
     */
    public function create_item_label($gquiz, $record = array()) {
        global $DB, $CFG;

        require_once($CFG->dirroot.'/mod/gquiz/lib.php');

        $itemobj = gquiz_get_item_class('label');
        $position = $DB->count_records('gquiz_item', array('gquiz' => $gquiz->id)) + 1;
     ...
     * @return int
     */
    public function create_item_multichoice($gquiz, $record = array()) {
        global $DB, $CFG;

        require_once($CFG->dirroot.'/mod/gquiz/lib.php');

        $itemobj = gquiz_get_item_class('multichoice');
        $position = $DB->count_records('gquiz_item', array('gquiz' => $gquiz->id)) + 1;
        ...
        $presentation = str_replace("\n", gquiz_MULTICHOICE_LINE_SEP, trim($record['values']));

        if ($record['horizontal'] == 1 AND $record['subtype'] != 'd') {
            $presentation .= gquiz_MULTICHOICE_ADJUST_SEP.'1';
        }
        $record['presentation'] = $record['subtype'].gquiz_MULTICHOICE_TYPE_SEP.$presentation;
     ...
     * @return int
     */
    public function create_item_multichoicerated($gquiz, $record = array()) {
        global $DB, $CFG;

        require_once($CFG->dirroot.'/mod/gquiz/lib.php');

        $itemobj = gquiz_get_item_class('multichoicerated');
        $position = $DB->count_records('gquiz_item', array('gquiz' => $gquiz->id)) + 1;
        ...
        $itemobj = new gquiz_item_multichoicerated();
        $presentation = $itemobj->prepare_presentation_values_save(trim($record['values']),
            gquiz_MULTICHOICERATED_VALUE_SEP2, gquiz_MULTICHOICERATED_VALUE_SEP);

        if ($record['horizontal'] == 1 AND $record['subtype'] != 'd') {
            $presentation .= gquiz_MULTICHOICERATED_ADJUST_SEP.'1';
        }
        $record['presentation'] = $record['subtype'].gquiz_MULTICHOICERATED_TYPE_SEP.$presentation;
     ...
     * @return int
     */
    public function create_item_numeric($gquiz, $record = array()) {
        global $DB, $CFG;

        require_once($CFG->dirroot.'/mod/gquiz/lib.php');

        $itemobj = gquiz_get_item_class('numeric');
        $position = $DB->count_records('gquiz_item', array('gquiz' => $gquiz->id)) + 1;
     ...
     * @return int
     */
    public function create_item_textarea($gquiz, $record = array()) {
        global $DB, $CFG;

        require_once($CFG->dirroot.'/mod/gquiz/lib.php');

        $itemobj = gquiz_get_item_class('textarea');
        $position = $DB->count_records('gquiz_item', array('gquiz' => $gquiz->id)) + 1;
     ...
     * @return int
     */
    public function create_item_textfield($gquiz, $record = array()) {
        global $DB, $CFG;

        require_once($CFG->dirroot.'/mod/gquiz/lib.php');

        $itemobj = gquiz_get_item_class('textfield');
        $position = $DB->count_records('gquiz_item', array('gquiz' => $gquiz->id)) + 1;
     ...
     */
    public function create_item_pagebreak($gquiz) {
        global $CFG;
        require_once($CFG->dirroot.'/mod/gquiz/lib.php');

        return gquiz_create_pagebreak($gquiz->id);
    }
}
