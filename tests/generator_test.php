 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_gquiz_generator_testcase extends advanced_testcase {
    ...
    public function test_create_instance() {
        ...
        $this->assertFalse($DB->record_exists('gquiz', array('course' => $course->id)));
        $gquiz = $this->getDataGenerator()->create_module('gquiz', array('course' => $course->id));
        $this->assertEquals(1, $DB->count_records('gquiz', array('course' => $course->id)));
        $this->assertTrue($DB->record_exists('gquiz', array('course' => $course->id)));
        $this->assertTrue($DB->record_exists('gquiz', array('id' => $gquiz->id)));

        $params = array('course' => $course->id, 'name' => 'One more gquiz');
        $gquiz = $this->getDataGenerator()->create_module('gquiz', $params);
        $this->assertEquals(2, $DB->count_records('gquiz', array('course' => $course->id)));
        $this->assertEquals('One more gquiz', $DB->get_field_select('gquiz', 'name', 'id = :id',
                array('id' => $gquiz->id)));
    }
    ...
    public function test_create_item_info() {
        ...
        $item1 = $gquizgenerator->create_item_info($gquiz);
        $item2 = $gquizgenerator->create_item_info($gquiz, array('name' => 'Custom name'));
        $records = $DB->get_records('gquiz_item', array('gquiz' => $gquiz->id), 'id');
    ...
    public function test_create_item_label() {
        ...
        $item1 = $gquizgenerator->create_item_label($gquiz);
        ...
        $item2 = $gquizgenerator->create_item_label($gquiz, array('presentation_editor' => $editor));
        $records = $DB->get_records('gquiz_item', array('gquiz' => $gquiz->id), 'id');
        $this->assertCount(2, $records);
        $this->assertEquals($item1->id, $records[$item1->id]->id);
        $this->assertEquals($item2->id, $records[$item2->id]->id);
    ...
    public function test_create_item_multichoice() {
        ...
        $item1 = $gquizgenerator->create_item_multichoice($gquiz);
        $item2 = $gquizgenerator->create_item_multichoice($gquiz, array('values' => "1\n2\n3\n4\n5", 'horizontal' => 1));
        $records = $DB->get_records('gquiz_item', array('gquiz' => $gquiz->id), 'id');
        $this->assertCount(2, $records);
        $this->assertEquals($item1->id, $records[$item1->id]->id);
        $this->assertEquals($item2->id, $records[$item2->id]->id);
    ...
    public function test_create_item_multichoicerated() {
        ...
        $item1 = $gquizgenerator->create_item_multichoicerated($gquiz);
        $item2 = $gquizgenerator->create_item_multichoicerated($gquiz, array(
                    'values' => "0/1\n1/2\n2/3\n3/4\n4/5", 'horizontal' => 1));
        $records = $DB->get_records('gquiz_item', array('gquiz' => $gquiz->id), 'id');
        $this->assertCount(2, $records);
        $this->assertEquals($item1->id, $records[$item1->id]->id);
    ...
    public function test_create_item_numeric() {
        ...
        $item2 = $gquizgenerator->create_item_numeric($gquiz, array('rangefrom' => '0', 'rangeto' => '10'));
        $records = $DB->get_records('gquiz_item', array('gquiz' => $gquiz->id), 'id');
        $this->assertCount(2, $records);
        $this->assertEquals($item1->id, $records[$item1->id]->id);
        $this->assertEquals($item2->id, $records[$item2->id]->id);
    ...
    public function test_create_item_textarea() {
        ...
        $item1 = $gquizgenerator->create_item_textarea($gquiz);
        $item2 = $gquizgenerator->create_item_textarea($gquiz, array('itemwidth' => '20', 'itemheight' => '10'));
        $records = $DB->get_records('gquiz_item', array('gquiz' => $gquiz->id), 'id');
        $this->assertCount(2, $records);
        $this->assertEquals($item1->id, $records[$item1->id]->id);
        $this->assertEquals($item2->id, $records[$item2->id]->id);
    ...
    public function test_create_item_textfield() {
        ...
        $item1 = $gquizgenerator->create_item_textfield($gquiz);
        $item2 = $gquizgenerator->create_item_textfield($gquiz, array('itemsize' => '20', 'itemmaxlength' => '10'));
        $records = $DB->get_records('gquiz_item', array('gquiz' => $gquiz->id), 'id');
        $this->assertCount(2, $records);
        $this->assertEquals($item1->id, $records[$item1->id]->id);
        $this->assertEquals($item2->id, $records[$item2->id]->id);
    ...
    public function test_create_item_pagebreak() {
        ...
        // Try to create a page break in an empty gquiz (no items).
        $gquizgenerator->create_item_pagebreak($gquiz);
        $records = $DB->get_records('gquiz_item', array('gquiz' => $gquiz->id), 'id');
        $this->assertCount(0, $records);

        // Create at least one item before the pagebreak.
        $gquizgenerator->create_item_textfield($gquiz);

        // Now, create one pagebreak.
        $item1 = $gquizgenerator->create_item_pagebreak($gquiz);
        $records = $DB->get_records('gquiz_item', array('gquiz' => $gquiz->id), 'id');
        $this->assertCount(2, $records);

        // This pagebreak won't be created (there is already one in the last position).
        $item2 = $gquizgenerator->create_item_pagebreak($gquiz);
