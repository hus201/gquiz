...
/**
 * Unit tests for (some of) mod/gquiz/classes/lib.php.
 *
 * @package    mod_gquiz
...
global $CFG;
require_once($CFG->dirroot . '/mod/gquiz/classes/completion.php');

/**
 * Unit tests for (some of) mod/gquiz/classes/completion.php.
 *
 * @copyright  2019 Tobias Reischmann
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_gquiz_completion_testcase extends advanced_testcase {
    /**
     * Returns the number of pages with visible elements for the current state of the gquiz completion.
     * @param mod_gquiz_completion $completion
     * @return int number of pages with at least one visible item.
     */
    private function get_number_of_visible_pages(mod_gquiz_completion $completion) {
    ...
    public function test_get_pages() {
        ...
        $gquiz = $this->getDataGenerator()->create_module('gquiz',
        ...
        $completion = new mod_gquiz_completion($gquiz, $cm, $course,
            false, null, $user1->id);
        ...
        $completion2 = new mod_gquiz_completion($gquiz, $cm, $course,
            false, null, $user2->id);
