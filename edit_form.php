...
class gquiz_edit_use_template_form extends moodleform {
    ...
    public function definition() {
        ...
        $elementgroup = array();
        //headline
        $mform->addElement('header', 'using_templates', get_string('using_templates', 'gquiz'));
        // hidden elements
        ...
        $owntemplates = gquiz_get_template_list($course, 'own');
        $publictemplates = gquiz_get_template_list($course, 'public');
        ...
        if ($owntemplates or $publictemplates) {
            ...
            if ($publictemplates) {
                ...
                $options[get_string('public', 'gquiz')] = $publicoptions;
            }
            ...
            $elementgroup[] = $mform->createElement('selectgroups',
                                                     'templateid',
                                                     get_string('using_templates', 'gquiz'),
                                                     $options,
                                                     $attributes);

            $elementgroup[] = $mform->createElement('submit',
                                                     'use_template',
                                                     get_string('use_this_template', 'gquiz'),
                                                     array('class' => 'hiddenifjs'));

            $mform->addGroup($elementgroup, 'elementgroup', '', array(' '), false);
        } else {
            $mform->addElement('static', 'info', get_string('no_templates_available_yet', 'gquiz'));
        }
...
class gquiz_edit_create_template_form extends moodleform {
    ...
    public function definition() {
        ...
        //headline
        $mform->addElement('header', 'creating_templates', get_string('creating_templates', 'gquiz'));
        ...
        $elementgroup[] = $mform->createElement('text',
                                                 'templatename',
                                                 get_string('name', 'gquiz'),
                                                 array('size'=>'40', 'maxlength'=>'200'));

        if (has_capability('mod/gquiz:createpublictemplate', context_system::instance())) {
            $elementgroup[] = $mform->createElement('checkbox',
                                                     'ispublic', '',
                                                     get_string('public', 'gquiz'));
        }
        ...
        // Buttons.
        $mform->addElement('submit', 'create_template', get_string('save_as_new_template', 'gquiz'));
    ...
    public function validation($data, $files) {
        ...
        if (!isset($data['templatename']) || trim(strval($data['templatename'])) === '') {
            $errors['elementgroup'] = get_string('name_required', 'gquiz');
        }
        return $errors;
    }
}
