...
/**
 * Contains class mod_gquiz_complete_form
 *
 * @package   mod_gquiz
...
/**
 * Class mod_gquiz_complete_form
 *
 * @package   mod_gquiz
 * @copyright 2016 Marina Glancy
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_gquiz_complete_form extends moodleform {
    ...
    /** @var int */
    protected $mode;
    /** @var mod_gquiz_structure|mod_gquiz_completion */
    protected $structure;
    /** @var mod_gquiz_completion */
     ...
     *
     * @param int $mode
     * @param mod_gquiz_structure $structure
     * @param string $formid CSS id attribute of the form
     * @param array $customdata
     */
    public function __construct($mode, mod_gquiz_structure $structure, $formid, $customdata = null) {
        $this->mode = $mode;
        ...
        parent::__construct(null, $customdata, 'POST', '',
                array('id' => $formid, 'class' => 'gquiz_form' . $isanonymous), true);
    ...
    public function definition() {
        ...
        if (isloggedin() && !isguestuser() && $this->mode != self::MODE_EDIT && $this->mode != self::MODE_VIEW_TEMPLATE &&
            ...
            if ($this->structure->is_anonymous()) {
                $anonymousmodeinfo = get_string('anonymous', 'gquiz');
            } else {
                $anonymousmodeinfo = get_string('non_anonymous', 'gquiz');
            }
            $element = $mform->addElement('static', 'anonymousmode', '',
                    get_string('mode', 'gquiz') . ': ' . $anonymousmodeinfo);
            $element->setAttributes($element->getAttributes() + ['class' => 'gquiz_mode']);
        }

        // Add buttons to go to previous/next pages and submit the gquiz.
        if ($this->mode == self::MODE_COMPLETE) {
            $buttonarray = array();
            $buttonarray[] = &$mform->createElement('submit', 'gopreviouspage', get_string('previous_page', 'gquiz'));
            $buttonarray[] = &$mform->createElement('submit', 'gonextpage', get_string('next_page', 'gquiz'),
                    array('class' => 'form-submit'));
            $buttonarray[] = &$mform->createElement('submit', 'savevalues', get_string('save_entries', 'gquiz'),
                    array('class' => 'form-submit'));
     ...
     *
     * This will add only items from a current page to the gquiz and adjust the buttons
     */
    protected function definition_complete() {
        if (!$this->structure instanceof mod_gquiz_completion) {
    ...
    protected function definition_preview() {
        foreach ($this->structure->get_items() as $gquizitem) {
            $itemobj = gquiz_get_item_class($gquizitem->typ);
            $itemobj->complete_form_element($gquizitem, $this);
        }
    }
    ...
    /**
     * Can be used by the items to get the course id for which gquiz is taken
     *
     * This function returns 0 for gquizs that are located inside the courses.
     * $this->get_gquiz()->course will return the course where gquiz is located.
     * $this->get_current_course_id() will return the course where user was before taking the gquiz
     *
     * @return int
     */
    ...
    /**
     * Record from 'gquiz' table corresponding to the current gquiz
     * @return stdClass
     */
    public function get_gquiz() {
        return $this->structure->get_gquiz();
    }

    /**
     * Current gquiz mode, see constants on the top of this class
     * @return int
     */
    ...
    /**
     * Returns the course where user was before taking the gquiz.
     *
     * For gquizs inside the course it will be the same as $this->get_gquiz()->course.
     * For gquizs on the frontpage it will be the same as $this->get_course_id()
     *
     * @return int
     */
    public function get_current_course_id() {
        return $this->structure->get_courseid() ?: $this->get_gquiz()->course;
    }
    ...
    protected function get_suggested_class($item) {
        $class = "gquiz_itemlist gquiz-item-{$item->typ}";
        if ($item->dependitem) {
            $class .= " gquiz_is_dependent";
        }
        if ($item->typ !== 'pagebreak') {
            $itemobj = gquiz_get_item_class($item->typ);
            if ($itemobj->get_hasvalue()) {
                $class .= " gquiz_hasvalue";
            }
        }
        return $class;
    }
    ...
    /**
     * Adds an item number to the question name (if gquiz autonumbering is on)
     * @param stdClass $item
     * @param HTML_QuickForm_element $element
     */
    protected function add_item_number($item, $element) {
        if ($this->get_gquiz()->autonumbering && !empty($item->itemnr)) {
    ...
    protected function add_item_label($item, $element) {
        if (strlen($item->label) && ($this->mode == self::MODE_EDIT || $this->mode == self::MODE_VIEW_TEMPLATE)) {
            $name = get_string('nameandlabelformat', 'mod_gquiz',
    ...
    protected function guess_element_id($item, $element) {
        if (!$id = $element->getAttribute('id')) {
            $attributes = $element->getAttributes();
            $id = $attributes['id'] = 'gquiz_item_' . $item->id;
            $element->setAttributes($attributes);
        }
    ...
    protected function enhance_name_for_edit($item, $element) {
        ...
        $menu->set_constraint('.gquiz_form');
        $menu->set_alignment(action_menu::TR, action_menu::BR);
        $menu->set_menu_trigger(get_string('edit'));
        $menu->prioritise = true;

        $itemobj = gquiz_get_item_class($item->typ);
        $actions = $itemobj->edit_actions($item, $this->get_gquiz(), $this->get_cm());
        ...
        $name = html_writer::span('', 'itemdd', array('id' => 'gquiz_item_box_' . $item->id)) .
    ...
    public function display() {
        ...
        if ($this->mode == self::MODE_EDIT) {
            $PAGE->requires->js_call_amd('mod_gquiz/edit', 'setup');
        }
    }
}
