...
class provider implements
    ...
    public static function get_metadata(collection $collection) : collection {
        ...
        $collection->add_database_table('gquiz_completed', $completedfields, 'privacy:metadata:completed');
        $collection->add_database_table('gquiz_completedtmp', $completedfields, 'privacy:metadata:completedtmp');

        $valuefields = [
            'value' => 'privacy:metadata:value:value'
        ];

        $collection->add_database_table('gquiz_value', $valuefields, 'privacy:metadata:value');
        $collection->add_database_table('gquiz_valuetmp', $valuefields, 'privacy:metadata:valuetmp');
        ...
        $sql = "
            SELECT DISTINCT ctx.id
              ...
              JOIN {course_modules} cm
                ON cm.instance = fc.gquiz
               AND cm.module = m.id
              JOIN {context} ctx
                ON ctx.instanceid = cm.id
               AND ctx.contextlevel = :modlevel
             WHERE fc.userid = :userid";
        $params = ['gquiz' => 'gquiz', 'modlevel' => CONTEXT_MODULE, 'userid' => $userid];
        $contextlist = new contextlist();
        $contextlist->add_from_sql(sprintf($sql, 'gquiz_completed'), $params);
        $contextlist->add_from_sql(sprintf($sql, 'gquiz_completedtmp'), $params);
        return $contextlist;
    }
    ...
    public static function get_users_in_context(userlist $userlist) {
        ...
        $params = ['gquiz' => 'gquiz', 'modlevel' => CONTEXT_MODULE, 'contextid' => $context->id];

        $userlist->add_from_sql('userid', sprintf($sql, 'gquiz_completed'), $params);
        $userlist->add_from_sql('userid', sprintf($sql, 'gquiz_completedtmp'), $params);
    }
        ...
        foreach ($recordset as $record) {
            ...
            if (!isset($data->submissions[$id])) {
                $data->submissions[$id] = [
                    ...
                    'anonymousresponse' => transform::yesno($record->anonymousresponse == gquiz_ANONYMOUS_YES),
            ...
            $itemobj = gquiz_get_item_class($record->itemtyp);
        ...
        $completedsql = "
            SELECT fc.id
              ...
              JOIN {course_modules} cm
               ...
               AND cm.module = m.id
             WHERE cm.id = :cmid";
        $completedparams = ['cmid' => $context->instanceid, 'gquiz' => 'gquiz'];

        // Delete temp answers and submissions.
        $completedtmpids = $DB->get_fieldset_sql(sprintf($completedsql, 'gquiz_completedtmp'), $completedparams);
        if (!empty($completedtmpids)) {
            list($insql, $inparams) = $DB->get_in_or_equal($completedtmpids, SQL_PARAMS_NAMED);
            $DB->delete_records_select('gquiz_valuetmp', "completed $insql", $inparams);
            $DB->delete_records_select('gquiz_completedtmp', "id $insql", $inparams);
        }

        // Delete answers and submissions.
        $completedids = $DB->get_fieldset_sql(sprintf($completedsql, 'gquiz_completed'), $completedparams);
        if (!empty($completedids)) {
            list($insql, $inparams) = $DB->get_in_or_equal($completedids, SQL_PARAMS_NAMED);
            $DB->delete_records_select('gquiz_value', "completed $insql", $inparams);
            $DB->delete_records_select('gquiz_completed', "id $insql", $inparams);
        }
    }
    ...
    public static function delete_data_for_user(approved_contextlist $contextlist) {
        ...
        $completedsql = "
            SELECT fc.id
             ...
             WHERE fc.userid = :userid
               AND cm.id $insql";
        $completedparams = array_merge($inparams, ['userid' => $userid, 'gquiz' => 'gquiz']);
        ...
        $completedsql = "
            SELECT fc.id
             ...
             WHERE cm.id = :instanceid
               AND fc.userid $insql";
        $completedparams = array_merge($inparams, ['instanceid' => $context->instanceid, 'gquiz' => 'gquiz']);
    ...
    protected static function prepare_export_query(array $contextids, $userid) {
        ...
        $makefetchsql = function($istmp) use ($DB, $contextids, $userid) {
            ...
            $completedtbl = $istmp ? 'gquiz_completedtmp' : 'gquiz_completed';
            $valuetbl = $istmp ? 'gquiz_valuetmp' : 'gquiz_value';
        ...
        $sql = "
         ...
         LEFT JOIN {gquiz_value} fv
                ON fv.id = q.valueid AND q.istmp = 0
         LEFT JOIN {gquiz_valuetmp} fvt
