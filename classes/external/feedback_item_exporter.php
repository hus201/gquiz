...
/**
 * Class for exporting a gquiz item (question).
 *
 * @package    mod_gquiz
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class gquiz_item_exporter extends exporter {
    ...
    protected function get_other_values(renderer_base $output) {
        ...
        $itemobj = gquiz_get_item_class($this->data->typ);
        $values = array(
            'itemfiles' => array(),
        ...
        $fs = get_file_storage();
        $files = array();
        $itemfiles = $fs->get_area_files($context->id, 'mod_gquiz', 'item', $this->data->id, 'filename', false);
        if (!empty($itemfiles)) {
