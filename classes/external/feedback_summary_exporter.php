...
/**
 * Class for exporting partial gquiz data.
 *
 * @package    mod_gquiz
...
/**
 * Class for exporting partial gquiz data (some fields are only viewable by admins).
 *
 * @copyright  2017 Juan Leyva <juan@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class gquiz_summary_exporter extends exporter {
    ...
    protected static function define_properties() {
        return array(
            ...
            'course' => array(
                'type' => PARAM_INT,
                'description' => 'Course id this gquiz is part of.',
            ),
            'name' => array(
                'type' => PARAM_TEXT,
                'description' => 'gquiz name.',
            ),
            'intro' => array(
                'default' => '',
                'type' => PARAM_RAW,
                'description' => 'gquiz introduction text.',
            ),
            'introformat' => array(
                'choices' => array(FORMAT_HTML, FORMAT_MOODLE, FORMAT_PLAIN, FORMAT_MARKDOWN),
                'type' => PARAM_INT,
                'default' => FORMAT_MOODLE,
                'description' => 'gquiz intro text format.',
            ),
            'anonymous' => array(
                'type' => PARAM_INT,
                'description' => 'Whether the gquiz is anonymous.',
            ),
    ...
    protected function get_other_values(renderer_base $output) {
        ...
        $values['introfiles'] = external_util::get_area_files($context->id, 'mod_gquiz', 'intro', false, false);

        if (!empty($this->data->page_after_submit)) {
            $values['pageaftersubmitfiles'] = external_util::get_area_files($context->id, 'mod_gquiz', 'page_after_submit');
        }
