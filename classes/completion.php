...
/**
 * Contains class mod_gquiz_completion
 *
 * @package   mod_gquiz
...
/**
 * Collects information and methods about gquiz completion (either complete.php or show_entries.php)
 *
 * @package   mod_gquiz
 * @copyright 2016 Marina Glancy
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_gquiz_completion extends mod_gquiz_structure {
    /** @var stdClass */
    protected $completed;
    /** @var stdClass */
    ...
    /** @var mod_gquiz_complete_form the form used for completing the gquiz */
    protected $form = null;
    /** @var bool true when the gquiz has been completed during the request */
     ...
     * @param cm_info $cm course module object corresponding to the $gquiz
     *     (at least one of $gquiz or $cm is required)
     * @param int $courseid current course (for site gquizs only)
     * @param bool $iscompleted has gquiz been already completed? If yes either completedid or userid must be specified.
     * @param int $completedid id in the table gquiz_completed, may be omitted if userid is specified
     *     but it is highly recommended because the same user may have multiple responses to the same gquiz
     ...
     */
    public function __construct($gquiz, $cm, $courseid, $iscompleted = false, $completedid = null,
                                $nonanonymouseuserid = null, $userid = 0) {
        global $DB;

        parent::__construct($gquiz, $cm, $courseid, 0, $userid);
        // Make sure courseid is always set for site gquiz.
        if ($this->gquiz->course == SITEID && !$this->courseid) {
        ...
        if ($iscompleted) {
            ...
            if (!$nonanonymouseuserid && !$completedid) {
                throw new coding_exception('Either $completedid or $nonanonymouseuserid must be specified for completed gquizs');
            }
            if ($completedid) {
                $params['id'] = $completedid;
            }
            if ($nonanonymouseuserid) {
                // We must respect the anonymousity of the reply that the user saw when they were completing the gquiz,
                // not the current state that may have been changed later by the teacher.
                $params['anonymous_response'] = gquiz_ANONYMOUS_NO;
                $params['userid'] = $nonanonymouseuserid;
            }
            $this->completed = $DB->get_record('gquiz_completed', $params, '*', MUST_EXIST);
            $this->courseid = $this->completed->courseid;
        }
    }

    /**
     * Returns a record from 'gquiz_completed' table
     * @return stdClass
     */
    ...
    /**
     * Check if the gquiz was just completed.
     *
     * @return bool true if the gquiz was just completed.
     * @since  Moodle 3.3
     */
     ...
     *
     * @return stdClass|false record from gquiz_completedtmp or false if not found
     */
    public function get_current_completed_tmp() {
        global $DB, $USER;
        if ($this->completedtmp === null) {
            $params = array('gquiz' => $this->get_gquiz()->id);
            ...
            $this->completedtmp = $DB->get_record('gquiz_completedtmp', $params);
        }
    ...
    protected function can_see_item($item) {
        ...
        $itemobj = gquiz_get_item_class($ditem->typ);
        if ($this->iscompleted) {
    ...
    protected function dependency_has_error($item) {
        ...
        if ($index1 >= $index2) {
            // Dependent item is after the current item in the gquiz.
            return true;
        }
    ...
    /**
     * Returns a value stored for this item in the gquiz (temporary or not, depending on the mode)
     * @param stdClass $item
     * @return string
     */
     ...
     *
     * @return array the responses (from the gquiz_valuetmp table)
     * @since  Moodle 3.3
     */
    public function get_unfinished_responses() {
        ...
        if ($completedtmp) {
            $responses = $DB->get_records('gquiz_valuetmp', ['completed' => $completedtmp->id]);
        }
        return $responses;
    }

    /**
     * Returns all temporary values for this gquiz or just a value for an item
     * @param stdClass $item
     * @return array
     */
     ...
     *
     * @return array the responses (from the gquiz_value table)
     * @since  Moodle 3.3
     */
    public function get_finished_responses() {
        ...
        if ($this->completed) {
            $responses = $DB->get_records('gquiz_value', ['completed' => $this->completed->id]);
        }
        return $responses;
    }

    /**
     * Returns all completed values for this gquiz or just a value for an item
     * @param stdClass $item
     * @return array
     */
    ...
    /**
     * Splits the gquiz items into pages
     *
    ...
    /**
     * Get the next page for the gquiz
     *
    ...
    public function get_next_page($gopage, $strictcheck = true) {
        ...
        // No further pages in the gquiz have any visible items.
        return null;
    }

    /**
     * Get the previous page for the gquiz
     *
    ...
    /**
     * Page index to resume the gquiz
     *
     * When user abandones answering gquiz and then comes back to it we should send him
    ...
    /**
     * Creates a new record in the 'gquiz_completedtmp' table for the current user/guest session
     *
     * @return stdClass record from gquiz_completedtmp or false if not found
     */
    protected function create_current_completed_tmp() {
        global $DB, $USER;
        $record = (object)['gquiz' => $this->gquiz->id];
        ...
        $record->anonymous_response = $this->gquiz->anonymous;
        $id = $DB->insert_record('gquiz_completedtmp', $record);
        $this->completedtmp = $DB->get_record('gquiz_completedtmp', ['id' => $id]);
        $this->valuestmp = null;
        return $this->completedtmp;
    }

    /**
     * If user has already completed the gquiz, create the temproray values from last completed attempt
     *
     * @return stdClass record from gquiz_completedtmp or false if not found
     */
    public function create_completed_tmp_from_last_completed() {
        if (!$this->get_current_completed_tmp()) {
            ...
            if ($lastcompleted) {
                $this->completedtmp = gquiz_set_tmp_values($lastcompleted);
            }
        }
        return $this->completedtmp;
    }
     ...
     *
     * @param stdClass $data data from the form mod_gquiz_complete_form
     */
    public function save_response_tmp($data) {
        ...
        if (!$completedtmp = $this->get_current_completed_tmp()) {
            ...
            $currentime = time();
            $DB->update_record('gquiz_completedtmp',
                    ['id' => $completedtmp->id, 'timemodified' => $currentime]);
            $completedtmp->timemodified = $currentime;
        }

        // Find all existing values.
        $existingvalues = $DB->get_records_menu('gquiz_valuetmp',
                ['completed' => $completedtmp->id], '', 'item, id');

        // Loop through all gquiz items and save the ones that are present in $data.
        $allitems = $this->get_items();
        foreach ($allitems as $item) {
            ...
            // Convert the value to string that can be stored in 'gquiz_valuetmp' or 'gquiz_value'.
            $itemobj = gquiz_get_item_class($item->typ);
            $newvalue['value'] = $itemobj->create_value($data->$keyname);

            // Update or insert the value in the 'gquiz_valuetmp' table.
            if (array_key_exists($item->id, $existingvalues)) {
                $newvalue['id'] = $existingvalues[$item->id];
                $DB->update_record('gquiz_valuetmp', $newvalue);
            } else {
                $DB->insert_record('gquiz_valuetmp', $newvalue);
            }
        }
    ...
    public function save_response() {
        ...
        $gquizcompleted = $this->find_last_completed();
        $gquizcompletedtmp = $this->get_current_completed_tmp();

        if (gquiz_check_is_switchrole()) {
        ...
        if ($this->gquiz->anonymous == gquiz_ANONYMOUS_NO) {
            ...
            gquiz_send_email_anonym($this->cm, $this->gquiz, $this->cm->get_course());
        }

        unset($SESSION->gquiz->is_started);

        // Update completion state.
        $completion = new completion_info($this->cm->get_course());
        if ((isloggedin() || $USER->id != $this->userid) && $completion->is_enabled($this->cm) &&
                $this->cm->completion == COMPLETION_TRACKING_AUTOMATIC && $this->gquiz->completionsubmit) {
    ...
    protected function delete_completedtmp() {
        ...
        if ($completedtmp = $this->get_current_completed_tmp()) {
            $DB->delete_records('gquiz_valuetmp', ['completed' => $completedtmp->id]);
            $DB->delete_records('gquiz_completedtmp', ['id' => $completedtmp->id]);
     ...
     *
     * @return stdClass record from gquiz_completed or false if not found
     */
    public function find_last_completed() {
        global $DB, $USER;
        if ((!isloggedin() && $USER->id == $this->userid) || isguestuser($this->userid)) {
            // Not possible to retrieve completed gquiz for guests.
            return false;
        }
        if ($this->is_anonymous()) {
            // Not possible to retrieve completed anonymous gquiz.
            return false;
        }
        $params = array('gquiz' => $this->gquiz->id,
            'userid' => $this->userid,
        ...
        $this->completed = $DB->get_record('gquiz_completed', $params);
        return $this->completed;
    }

    /**
     * Checks if user has capability to submit the gquiz
     *
     * There is an exception for fully anonymous gquizs when guests can complete
     * gquiz without the proper capability.
     *
     * This should be followed by checking {@link can_submit()} because even if
     * user has capablity to complete, they may have already submitted gquiz
    ...
    public function can_complete() {
        ...
        if (has_capability('mod/gquiz:complete', $context, $this->userid)) {
            return true;
        }

        if (!empty($CFG->gquiz_allowfullanonymous)
                    AND $this->gquiz->course == SITEID
                    AND $this->gquiz->anonymous == gquiz_ANONYMOUS_YES
                    AND ((!isloggedin() && $USER->id == $this->userid) || isguestuser($this->userid))) {
            // Guests are allowed to complete fully anonymous gquiz without having 'mod/gquiz:complete' capability.
            return true;
        }
    ...
    public function can_submit() {
        if ($this->get_gquiz()->multiple_submit == 0 ) {
    ...
    public function trigger_module_viewed() {
        $event = \mod_gquiz\event\course_module_viewed::create_from_record($this->gquiz, $this->cm, $this->cm->get_course());
        $event->trigger();
    }
    ...
    /**
     * Process a page jump via the mod_gquiz_complete_form.
     *
    ...
    public function process_page($gopage, $gopreviouspage = false) {
        ...
        $this->form = new mod_gquiz_complete_form(mod_gquiz_complete_form::MODE_COMPLETE,
            $this, 'gquiz_complete_form', array('gopage' => $gopage));

        if ($this->form->is_cancelled()) {
            // Form was cancelled - return to the course page.
            $urltogo = course_get_url($this->courseid ?: $this->gquiz->course);
            ...
            if (!isset($SESSION->gquiz->is_started) OR !$SESSION->gquiz->is_started == true) {
            ...
            if (!empty($data->savevalues) || !empty($data->gonextpage)) {
                if (($nextpage = $this->get_next_page($gopage)) !== null) {
                    ...
                    $this->save_response();
                    if (!$this->get_gquiz()->page_after_submit) {
                        \core\notification::success(get_string('entries_saved', 'gquiz'));
                    }
