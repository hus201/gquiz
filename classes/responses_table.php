...
/**
 * Contains class mod_gquiz_responses_table
 *
 * @package   mod_gquiz
...
/**
 * Class mod_gquiz_responses_table
 *
 * @package   mod_gquiz
 * @copyright 2016 Marina Glancy
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_gquiz_responses_table extends table_sql {

    /**
     * Maximum number of gquiz questions to display in the "Show responses" table
     */
    const PREVIEWCOLUMNSLIMIT = 10;

    /**
     * Maximum number of gquiz questions answers to retrieve in one SQL query.
     ...
     */
    public function __construct(mod_gquiz_structure $gquizstructure, $group = 0) {
        $this->gquizstructure = $gquizstructure;

        parent::__construct('gquiz-showentry-list-' . $gquizstructure->get_cm()->instance);

        $this->showall = optional_param($this->showallparamname, 0, PARAM_BOOL);
        $this->define_baseurl(new moodle_url('/mod/gquiz/show_entries.php',
            ['id' => $this->gquizstructure->get_cm()->id]));
        if ($courseid = $this->gquizstructure->get_courseid()) {
        ...
        $name = format_string($gquizstructure->get_gquiz()->name);
        $this->is_downloadable(true);
        $this->is_downloading(optional_param($this->downloadparamname, 0, PARAM_ALPHA),
                $name, get_string('responses', 'gquiz'));
    ...
    protected function init($group = 0) {
        ...
        $params['instance'] = $this->gquizstructure->get_gquiz()->id;
        $params['notdeleted'] = 0;
        $params['courseid'] = $this->gquizstructure->get_courseid();
    ...
    public function get_context(): context {
        return context_module::instance($this->gquizstructure->get_cm()->id);
    }
    ...
    public function other_cols($column, $row) {
        if (preg_match('/^val(\d+)$/', $column, $matches)) {
            $items = $this->gquizstructure->get_items();
            $itemobj = gquiz_get_item_class($items[$matches[1]]->typ);
    ...
    public function col_userpic($row) {
        ...
        return $OUTPUT->user_picture($user, array('courseid' => $this->gquizstructure->get_cm()->course));
    }
    ...
    public function col_deleteentry($row) {
        ...
        $deleteaction = new confirm_action(get_string('confirmdeleteentry', 'gquiz'));
        return $OUTPUT->action_icon($deleteentryurl,
            new pix_icon('t/delete', get_string('delete_entry', 'gquiz')), $deleteaction);
    }
    ...
    public function col_courseid($row) {
        $courses = $this->gquizstructure->get_completed_courses();
        $name = '';
    ...
    public function col_groups($row) {
        $groups = '';
        if ($usergrps = groups_get_all_groups($this->gquizstructure->get_cm()->course, $row->userid, 0, 'name')) {
    ...
    protected function add_all_values_to_output() {
        ...
        $items = $this->gquizstructure->get_items(true);
        ...
        foreach ($items as $nr => $item) {
            if ($columnscount++ < self::TABLEJOINLIMIT) {
                ...
                $this->sql->from .= " LEFT OUTER JOIN {gquiz_value} v{$nr} " .
            ...
            if (strval($item->label) !== '') {
                $columnheader = get_string('nameandlabelformat', 'mod_gquiz',
        ...
        if (!$this->is_downloading() && has_capability('mod/gquiz:deletesubmissions', $this->get_context())) {
    ...
    public function display() {
        global $OUTPUT;
        groups_print_activity_menu($this->gquizstructure->get_cm(), $this->baseurl->out());
        ...
        if (count($this->gquizstructure->get_items(true)) > self::PREVIEWCOLUMNSLIMIT) {
            echo $OUTPUT->notification(get_string('questionslimited', 'gquiz', self::PREVIEWCOLUMNSLIMIT), 'info');
        }

        $this->out($this->showall ? $grandtotal : gquiz_DEFAULT_PAGE_COUNT,
                $grandtotal > gquiz_DEFAULT_PAGE_COUNT);

        // Toggle 'Show all' link.
        if ($this->totalrows > gquiz_DEFAULT_PAGE_COUNT) {
            if (!$this->use_pages) {
                echo html_writer::div(html_writer::link(new moodle_url($this->baseurl, [$this->showallparamname => 0]),
                        get_string('showperpage', '', gquiz_DEFAULT_PAGE_COUNT)), 'showall');
            } else {
    ...
    public function get_reponse_navigation_links($record) {
        ...
        while ($this->rawdata->valid()) {
            ...
            if ($row->id == $record->id) {
                $page = $this->showall ? 0 : floor($counter / gquiz_DEFAULT_PAGE_COUNT);
                $thisrow = $row;
                $this->rawdata->next();
    ...
    protected function build_table_chunk(&$rows, &$columnsgroups) {
        ...
        foreach ($columnsgroups as $columnsgroup) {
            ...
            foreach ($columnsgroup as $nr => $item) {
                ...
                $from .= " LEFT OUTER JOIN {gquiz_value} v{$nr} " .
                    "ON v{$nr}.completed = c.id AND v{$nr}.item = :itemid{$nr}";
