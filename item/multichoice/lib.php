...
define('gquiz_MULTICHOICE_TYPE_SEP', '>>>>>');
define('gquiz_MULTICHOICE_LINE_SEP', '|');
define('gquiz_MULTICHOICE_ADJUST_SEP', '<<<<<');
define('gquiz_MULTICHOICE_IGNOREEMPTY', 'i');
define('gquiz_MULTICHOICE_HIDENOSELECT', 'h');
...
class gquiz_item_multichoice extends gquiz_item_base {
    ...
    public function build_editform($item, $gquiz, $cm) {
        ...
        //get the lastposition number of the gquiz_items
        $position = $item->position;
        $lastposition = $DB->count_records('gquiz_item', array('gquiz'=>$gquiz->id));
        if ($position == -1) {
        ...
        $gquizitems = gquiz_get_depend_candidates_for_item($gquiz, $item);
        ...
        //Added by gquiz End
        $this->item_form = new gquiz_multichoice_form('edit_item.php', $customdata);
    }
     ...
     *
     * @param stdClass $item the db-object from gquiz_item
     * @param int $groupid
     * @param int $courseid
     * @return array
     */
    protected function get_analysed($item, $groupid = false, $courseid = false) {
        ...
        $answers = null;
        $answers = explode (gquiz_MULTICHOICE_LINE_SEP, $info->presentation);
        if (!is_array($answers)) {
            return null;
        }

        //get the values
        $values = gquiz_get_group_values($item, $groupid, $courseid, $this->ignoreempty($item));
        if (!$values) {
            return null;
        }
        ...
        if ($info->subtype == 'c') {
            ...
            for ($i = 1; $i <= $sizeofanswers; $i++) {
                ...
                foreach ($values as $value) {
                    ...
                    $vallist = explode(gquiz_MULTICHOICE_LINE_SEP, $value->value);
    ...
    public function get_printval($item, $value) {
        ...
        $presentation = explode (gquiz_MULTICHOICE_LINE_SEP, $info->presentation);

        if ($info->subtype == 'c') {
            $vallist = array_values(explode (gquiz_MULTICHOICE_LINE_SEP, $value->value));
    ...
    public function print_analysed($item, $itemnr = '', $groupid = false, $courseid = false) {
        ...
        if ($analysed_item) {
            ...
            $series = new \core\chart_series(format_string(get_string("responses", "gquiz")), $data['series']);
    ...
    protected function get_options($item) {
        ...
        if ($info->subtype === 'r' && !$this->hidenoselect($item)) {
            $options = array(0 => get_string('not_selected', 'gquiz')) + $options;
        }
     ...
     *
     * @param stdClass $item
     * @param mod_gquiz_complete_form $form
     */
    public function complete_form_element($item, $form) {
        ...
        if ($info->subtype === 'd' || ($info->subtype === 'r' && $form->is_frozen())) {
            ...
            $objs = [];
            foreach (explode(gquiz_MULTICHOICE_LINE_SEP, $form->get_item_value($item)) as $v) {
            ...
            if ($info->subtype === 'c') {
                ...
                $objs[] = ['static', '', '', html_writer::span('', '', ['id' => 'gquiz_item_' . $item->id])];
                $element = $form->add_form_group_element($item, 'group_'.$inputname, $name, $objs, $separator, $class);
                if ($tmpvalue) {
                    foreach (explode(gquiz_MULTICHOICE_LINE_SEP, $tmpvalue) as $v) {
    ...
    public function create_value($value) {
        ...
        return join(gquiz_MULTICHOICE_LINE_SEP, $value);
    }
    ...
    public function compare_value($item, $dbvalue, $dependvalue) {
        ...
        if (is_array($dbvalue)) {
            ...
            $dbvalues = explode(gquiz_MULTICHOICE_LINE_SEP, $dbvalue);
        }
    ...
    public function get_info($item) {
        ...
        $parts = explode(gquiz_MULTICHOICE_TYPE_SEP, $item->presentation);
        ...
        if ($info->subtype != 'd') {
            $parts = explode(gquiz_MULTICHOICE_ADJUST_SEP, $info->presentation);
    ...
    public function set_ignoreempty($item, $ignoreempty=true) {
        $item->options = str_replace(gquiz_MULTICHOICE_IGNOREEMPTY, '', $item->options);
        if ($ignoreempty) {
            $item->options .= gquiz_MULTICHOICE_IGNOREEMPTY;
        }
    }

    public function ignoreempty($item) {
        if (strstr($item->options, gquiz_MULTICHOICE_IGNOREEMPTY)) {
            return true;
        }
        return false;
    }
    ...
    public function set_hidenoselect($item, $hidenoselect=true) {
        $item->options = str_replace(gquiz_MULTICHOICE_HIDENOSELECT, '', $item->options);
        if ($hidenoselect) {
            $item->options .= gquiz_MULTICHOICE_HIDENOSELECT;
        }
    }

    public function hidenoselect($item) {
        if (strstr($item->options, gquiz_MULTICHOICE_HIDENOSELECT)) {
            return true;
        }
        return false;
    }
