...
class gquiz_item_captcha extends gquiz_item_base {
    ...
    public function build_editform($item, $gquiz, $cm) {
        ...
        if (isset($item->id) AND $item->id > 0) {
            notice(get_string('there_are_no_settings_for_recaptcha', 'gquiz'), $editurl->out());
            exit;
        }

        // Only one recaptcha can be in a gquiz.
        $params = array('gquiz' => $gquiz->id, 'typ' => $this->type);
        if ($DB->record_exists('gquiz_item', $params)) {
            notice(get_string('only_one_captcha_allowed', 'gquiz'), $editurl->out());
            exit;
        }
        ...
        $this->item->gquiz = $gquiz->id;
        $this->item->template = 0;
        $this->item->name = get_string('captcha', 'gquiz');
        $this->item->label = '';
    ...
    public function save_item() {
        ...
        if (empty($this->item->id)) {
            $this->item->id = $DB->insert_record('gquiz_item', $this->item);
        } else {
            $DB->update_record('gquiz_item', $this->item);
        }

        return $DB->get_record('gquiz_item', array('id'=>$this->item->id));
    }
    ...
    public function get_display_name($item, $withpostfix = true) {
        return get_string('captcha', 'gquiz');
    }
    ...
    public function complete_form_element($item, $form) {
        ...
        if ($form->get_mode() != mod_gquiz_complete_form::MODE_COMPLETE) {
            ...
            $form->add_form_element($item,
                    ['static', $inputname, $name, html_writer::span('', '', ['id' => 'gquiz_item_' . $item->id])],
                    false,
                    false);
        } else {
    ...
    public function edit_actions($item, $gquiz, $cm) {
        $actions = parent::edit_actions($item, $gquiz, $cm);
