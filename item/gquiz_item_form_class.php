...
define('gquiz_ITEM_NAME_TEXTBOX_SIZE', 80);
define('gquiz_ITEM_LABEL_TEXTBOX_SIZE', 20);
abstract class gquiz_item_form extends moodleform {
    public function definition() {
        ...
        if (array_filter(array_keys($common['items']))) {
            $mform->addElement('select',
                                'dependitem',
                                get_string('dependitem', 'gquiz').'&nbsp;',
                                $common['items']
                                );
            $mform->addHelpButton('dependitem', 'depending', 'gquiz');
            $mform->addElement('text',
                                'dependvalue',
                                get_string('dependvalue', 'gquiz'),
                                array('size'=>gquiz_ITEM_LABEL_TEXTBOX_SIZE, 'maxlength'=>255));
        ...
        $mform->addElement('hidden', 'gquiz', $common['gquiz']);
        $mform->setType('gquiz', PARAM_INT);
        ...
        if (!empty($item->id)) {
            $buttonarray[] = &$mform->createElement('submit',
                                                    'update_item',
                                                    get_string('update_item', 'gquiz'));

            $buttonarray[] = &$mform->createElement('submit',
                                                    'clone_item',
                                                    get_string('save_as_new_item', 'gquiz'));
        } else {
            $mform->addElement('hidden', 'clone_item', 0);
            $mform->setType('clone_item', PARAM_INT);
            $buttonarray[] = &$mform->createElement('submit',
                                                    'save_item',
                                                    get_string('save_item', 'gquiz'));
        }
