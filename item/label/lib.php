...
class gquiz_item_label extends gquiz_item_base {
    ...
    public function build_editform($item, $gquiz, $cm) {
        ...
        $this->item_form = new gquiz_label_form('edit_item.php', $customdata);
    }
    ...
    private function print_item($item) {
        ...
        if (!$item->gquiz AND $item->template) {
            $template = $DB->get_record('gquiz_template', array('id'=>$item->template));
            ...
            $cm = get_coursemodule_from_instance('gquiz', $item->gquiz);
