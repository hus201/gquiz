...
class gquiz_item_info extends gquiz_item_base {
    protected $type = "info";

    /** Mode recording response time (for non-anonymous gquizs only) */
    ...
    public function build_editform($item, $gquiz, $cm) {
        ...
        if ($gquiz->anonymous == gquiz_ANONYMOUS_NO || $item->presentation == self::MODE_RESPONSETIME) {
            // "Response time" is hidden anyway in case of anonymous gquiz, no reason to offer this option.
            // However if it was already selected leave it in the dropdown.
            $presentationoptions[self::MODE_RESPONSETIME] = get_string('responsetime', 'gquiz');
        }
        $presentationoptions[self::MODE_COURSE]  = get_string('course');
        $presentationoptions[self::MODE_CATEGORY]  = get_string('coursecategory');

        //build the form
        $this->item_form = new gquiz_info_form('edit_item.php',
                                                  array('item'=>$item,
                                                  'common'=>$commonparams,
    ...
    protected function get_analysed($item, $groupid = false, $courseid = false) {
        ...
        $values = gquiz_get_group_values($item, $groupid, $courseid);
        if ($values) {
            $data = array();
     ...
     * @return string
     */
    protected function get_current_value($item, $gquiz, $courseid) {
        global $DB;
        switch ($item->presentation) {
            case self::MODE_RESPONSETIME:
                if ($gquiz->anonymous != gquiz_ANONYMOUS_YES) {
                    // Response time is not allowed in anonymous gquizs.
                    return time();
                }
                break;
            case self::MODE_COURSE:
    ...
    public function complete_form_element($item, $form) {
        if ($form->get_mode() == mod_gquiz_complete_form::MODE_VIEW_RESPONSE) {
            $value = strval($form->get_item_value($item));
        } else {
            $value = $this->get_current_value($item,
                    $form->get_gquiz(), $form->get_current_course_id());
        }
        ...
        $element->freeze();
        if ($form->get_mode() == mod_gquiz_complete_form::MODE_COMPLETE) {
     ...
     * @param mixed $value element from mod_gquiz_complete_form::get_data() with the name $item->typ.'_'.$item->id
     * @return string
     */
    ...
    public function get_data_for_external($item) {
        global $DB;
        $gquiz = $DB->get_record('gquiz', array('id' => $item->gquiz), '*', MUST_EXIST);
        // Return the default value (course name, category name or timestamp).
        return $this->get_current_value($item, $gquiz, $gquiz->course);
    }
