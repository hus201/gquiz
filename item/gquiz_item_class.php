...
abstract class gquiz_item_base {
     ...
     */
    abstract public function build_editform($item, $gquiz, $cm);
     ...
     * @param object $item the db-object from gquiz_item
     ...
     *
     * @param $item the db-object from gquiz_item
     * @param string $itemnr
     * @param integer $groupid
     * @param integer $courseid
     ...
     *
     * @param object $item the db-object from gquiz_item
     * @param string $value a item-related value from gquiz_values
     * @return string
     */
     ...
     * - to display the form when user completes gquiz
     * - to display existing elements when teacher edits the gquiz items
     * - to display the gquiz preview (print.php)
     * - to display the completed response
     * - to preview a gquiz template
     *
     ...
     */
    public function edit_actions($item, $gquiz, $cm) {
        $actions = array();

        $strupdate = get_string('edit_item', 'gquiz');
        $actions['update'] = new action_menu_link_secondary(
            new moodle_url('/mod/gquiz/edit_item.php', array('id' => $item->id)),
        ...
        if ($this->can_switch_require()) {
            if ($item->required == 1) {
                $buttontitle = get_string('switch_item_to_not_required', 'gquiz');
                $buttonimg = 'required';
            } else {
                $buttontitle = get_string('switch_item_to_required', 'gquiz');
                $buttonimg = 'notrequired';
            }
            $actions['required'] = new action_menu_link_secondary(
                new moodle_url('/mod/gquiz/edit.php', array('id' => $cm->id,
                    'switchitemrequired' => $item->id, 'sesskey' => sesskey())),
                new pix_icon($buttonimg, $buttontitle, 'gquiz', array('class' => 'iconsmall', 'title' => '')),
                $buttontitle,
                array('class' => 'editing_togglerequired', 'data-action' => 'togglerequired')
            );
        }

        $strdelete = get_string('delete_item', 'gquiz');
        $actions['delete'] = new action_menu_link_secondary(
            new moodle_url('/mod/gquiz/edit.php', array('id' => $cm->id, 'deleteitem' => $item->id, 'sesskey' => sesskey())),
...
class gquiz_item_pagebreak extends gquiz_item_base {
    ...
    public function complete_form_element($item, $form) {
        $form->add_form_element($item,
            ['static',
                $item->typ.'_'.$item->id,
                '',
                html_writer::empty_tag('hr', ['class' => 'gquiz_pagebreak', 'id' => 'gquiz_item_' . $item->id])
            ]);
    }
    ...
    public function edit_actions($item, $gquiz, $cm) {
        $actions = array();
        $strdelete = get_string('delete_pagebreak', 'gquiz');
        $actions['delete'] = new action_menu_link_secondary(
            new moodle_url('/mod/gquiz/edit.php', array('id' => $cm->id, 'deleteitem' => $item->id, 'sesskey' => sesskey())),
            new pix_icon('t/delete', $strdelete, 'moodle', array('class' => 'iconsmall', 'title' => '')),
            $strdelete,
