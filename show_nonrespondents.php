...
$perpage = optional_param('perpage', gquiz_DEFAULT_PAGE_COUNT, PARAM_INT);  // how many per page
$showall = optional_param('showall', false, PARAM_INT);  // should we show all users
// $SESSION->gquiz->current_tab = $do_show;
...
//this page only can be shown on nonanonymous gquizs in courses
//we should never reach this page
if ($gquiz->anonymous != gquiz_ANONYMOUS_NO OR $gquiz->course == SITEID) {
    print_error('error');
}

$url = new moodle_url('/mod/gquiz/show_nonrespondents.php', array('id'=>$cm->id));
...
if ($action == 'sendmessage' AND $canbulkmessaging) {
    ...
    $link2 = $CFG->wwwroot.'/mod/gquiz/index.php?id='.$course->id;
    $link3 = $CFG->wwwroot.'/mod/gquiz/view.php?id='.$cm->id;

    $htmlmessage .= '<div class="navbar">'.
    '<a target="_blank" href="'.$link1.'">'.$shortname.'</a> &raquo; '.
    '<a target="_blank" href="'.$link2.'">'.$strgquizs.'</a> &raquo; '.
    '<a target="_blank" href="'.$link3.'">'.format_string($gquiz->name, true).'</a>'.
    '</div>';
    ...
    if (is_array($messageuser)) {
        foreach ($messageuser as $userid) {
            ...
            $eventdata->contexturlname   = $gquiz->name;
...
$baseurl = new moodle_url('/mod/gquiz/show_nonrespondents.php');
...
if ($canbulkmessaging) {
    ...
    $mastercheckbox = new \core\output\checkbox_toggleall('gquiz-non-respondents', true, [
        'id' => $selectallid,
        'name' => $selectallid,
        'value' => 1,
...
$table = new flexible_table('gquiz-shownonrespondents-'.$course->id);
...
$matchcount = gquiz_count_incomplete_users($cm, $usedgroupid);
...
// Return students record including if they started or not the gquiz.
$students = gquiz_get_incomplete_users($cm, $usedgroupid, $sort, $startpage, $pagecount, true);
//####### viewreports-start
//print the list of students
echo $OUTPUT->heading(get_string('non_respondents_students', 'gquiz', $matchcount), 4);
...
if (empty($students)) {
    ...
    if ($canbulkmessaging) {
        echo '<form class="mform" action="show_nonrespondents.php" method="post" id="gquiz_sendmessageform">';
    }
    ...
    foreach ($students as $student) {
        ...
        if ($student->gquizstarted) {
            $data[] = get_string('started', 'gquiz');
        } else {
            $data[] = get_string('not_started', 'gquiz');
        }

        //selections to bulk messaging
        if ($canbulkmessaging) {
            $checkbox = new \core\output\checkbox_toggleall('gquiz-non-respondents', false, [
                'id' => 'messageuser-' . $student->id,
                'name' => 'messageuser[]',
                'classes' => 'mr-1',
                'value' => $student->id,
                'label' => get_string('includeuserinrecipientslist', 'mod_gquiz', fullname($student)),
    ...
    if ($showall) {
        ...
        echo $OUTPUT->container(html_writer::link($allurl, get_string('showperpage', '', gquiz_DEFAULT_PAGE_COUNT)),
                                    array(), 'showall');
    ...
    if ($canbulkmessaging) {
        ...
        echo '<legend class="ftoggler">'.get_string('send_message', 'gquiz').'</legend>';
        echo '<div>';
        echo '<label for="gquiz_subject">'.get_string('subject', 'gquiz').'&nbsp;</label>';
        echo '<input type="text" id="gquiz_subject" size="50" maxlength="255" name="subject" value="'.s($subject).'" />';
        echo '</div>';
        echo $OUTPUT->print_textarea('message', 'edit-message', $message, 15, 25);
        print_string('formathtml');
        echo '<input type="hidden" name="format" value="'.FORMAT_HTML.'" />';
        echo '<br /><div class="buttons">';
        echo '<input type="submit" name="send_message" value="'.get_string('send', 'gquiz').'" class="btn btn-secondary" />';
        echo '</div>';
